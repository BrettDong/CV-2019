...
void commit(int fd, const char *data, int sz)
{
    serialtimer = (double)cv::getTickCount();
    if((serialtimer - timerlast) / cv::getTickFrequency() > 1.0)
        return;
    for(int i = 0; i < sz && i < sizeof(Tdata); i++)
        Tdata[i] = static_cast<unsigned char>(data[i]);
    write(fd, Tdata, sz);
}
////////////////////////////////////////////////////////////////////////////////
